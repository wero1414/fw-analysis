...
case "$1" in
  start)
    ...
    ;;
  stop)
    ubus call network.interface.wan set_data '{ "ddns_status": "disconnected" }' 2> /dev/null
    ubus call network.interface.wan set_data '{"ddns_ip":null}'
    if [ -n "$INTERFACE" ]; then
...
while : ; do
  ...
  ubus call network.interface.wan set_data '{ "ddns_ip": "'$LOCAL_IP'" }' 2> /dev/null
  ...
  SERVER_RESPONSE=$(awk '{print $1}' $DATFILE)
  ...
  if [ "$SERVER_RESPONSE" == "good" ] || [ "$SERVER_RESPONSE" == "nochg" ]; then
	  ubus call network.interface.wan set_data '{ "ddns_status": "connected" }' 2> /dev/null
  else
	  ubus call network.interface.wan set_data "{ 'ddns_status': '$SERVER_RESPONSE' }" 2> /dev/null
  fi
